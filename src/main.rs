...
//     // These cause panics automatically:
//     let v = [1, 2, 3];
//     v[99];    // index out of bounds -> panic

//     let x: u8 = 255;
//     let y = x + 1;  // overflow in debug mode -> panic
// }


// fn divide(a: f64, b: f64) -> Result<f64, String> {
//     if b == 0.0 {
//         Err(String::from("Cannot divide by 0.0"))
//     } else {
//         Ok(a / b)
//     }
// }
...
// fn main() {
//     // Handling with match — explicit and complete
//     match divide(10.0, 2.0) {
//         Ok(result) => println!("Result: {}", result),     // 5
...
fn parse_lamports(s: &str) -> Result<u64, String> {
    s.parse::<u64>().map_err(|e| format!("'{}' is not valid: {}", s, e))
}

fn main() {
    let good: Result<u64, String> = Ok(2_000_000_000);
    let bad:  Result<u64, String> = Err(String::from("Account not found"));

    // 1. is_ok() and is_err() — check without consuming
    println!("{}", good.is_ok());   // true
    println!("{}", bad.is_err());   // true

    // 2. unwrap_or — get value or use a default
    // println!("{}", bad.unwrap_or(0));  // 0

    // 3. unwrap_or_else — compute a default lazily
    ...
    // 4. map — transform the Ok value, pass Err through
    let doubled = good.map(|v| v * 2);
    println!("{:?}", doubled);   // Ok(4000000000)

    // 5. map_err — transform the Err value, pass Ok through
    let better_err = bad.map_err(|e| format!("FATAL: {}", e));
    println!("{:?}", better_err);

    // 6. and_then — chain operations that can fail
    let result = parse_lamports("1000000000")
        .and_then(|lamps| if lamps > 0 { Ok(lamps) } else { Err(String::from("Zero")) });
    println!("{:?}", result);   // Ok(1000000000)

    // DANGEROUS — only use when you are 100% certain it is Ok
    // good.unwrap()    ← panics if Err
    // good.expect("msg") ← panics with your message if Err
}